...
const RoleList = () => {
    ...
    const deleteRow = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const data = postData
                fetch(backend_url + 'delRole/?id=' + id + '&limit=' + data.limit + '&keyword=' + data.keyword + '&filter=' + data.filter + '&sorter=' + data.sorter + '&pagenum=' + data.pagenum, { method: 'GET' })
                    .then(response => response.json())
                    .then(data => {
    ...
    const editRow = (row) => {
        handleDetailShow();
        setDataValue({ id: row.id, name: row.name, permissionid: row.permissionid })
    }
    const addData = () => {
                    ...
                    <Modal show={show.flag} onHide={handleClose}>
                        ...
                        <Modal.Body className="modal-body"> <div className="p-4">
                            <Form className="form-horizontal">
                                ...
                                <FormGroup className="form-group">
                                    <select id="permissionid" placeholder="permission id" name="permissionid" className="form-control select2 form-select" value={dataValue.permissionid} onChange={(event) => setPropsValue(event)} >
                                        <option value={0} key={""}>No permission</option>
                                        {permissionList.data.map(e => {
                                            return <option value={e.id} key={e.id}>{e.name}</option>
                                        })}
